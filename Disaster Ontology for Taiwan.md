...
#### 災害防救法：奠定生命週期框架
台灣的《災害防救法》為全國的災害管理工作提供了核心的法律依據，其明確定義了災害管理的三个核心階段：災害預防、災害緊急應變、以及災後復原重建。此一法律框架不僅是台灣所有災害防救計畫的基石，也為本報告所提出的本體論提供了天然的、高層次的結構基礎。這種按時間序列劃分的階段，與日本的《防災基本計畫》所採用的「災害予防」、「災害応急対策」、「災害復旧・復興」三階段模型高度一致，證明了此種結構的普適性與有效性。
...
*   **關鍵基礎設施與設施 (Critical Infrastructure & Facilities)**：盤點醫院、學校、消防站、發電廠等重要設施。
    ...
    *   **OSM整合**：此類資料應與OSM同步。例如，一所醫院在OSM上應標記為 `amenity=hospital`，若其設有急診室，則應加上 `emergency=yes`。一個指定的避難所，如學校，則可使用 `emergency:*` 命名空間，標記為 `amenity=school` 加上 `emergency:shelter=yes`。本體論負責儲存與此地理物件相關的動態營運資料。
...
#### 採用「社群生命線」模型
為了更系統性地掌握災害衝擊，我們將圍繞美國NRF的七條「社群生命線」（Community Lifelines）來組織應變階段的資料。此模型不僅僅是一個分類方案，它是一個強大的分析工具，有助於理解現代災害中常見的「連鎖性失效」（cascading failures）。傳統的災損報告可能會將「停電」和「通訊中斷」列為獨立項目，但生命線模型強迫我們從系統角度思考：`能源`生命線的失效會直接導致`通訊`生命線的失效（因為基地台需要電力），進而癱瘓`健康與醫療`生命線（醫院無法通聯）和`安全保障`生命線（應變人員無法協調）。透過圍繞生命線來建構本體論，決策者可以從單純的災損編目，提升到基於相互依賴性來*預測和排定*應變措施的優先順序。這使得指揮官能提出更具策略性的問題：「恢復哪一條最關鍵的生命線，能對所有其他生命線產生最大的正面連鎖效應？」這是應變策略能力的重大飛躍。
...
*   **總體災情報告 (Overall Situation Report)**：事件的高層級摘要。
    ...
    *   **標準整合**：對應**EDXL-SitRep**標準，為正式的災情報告提供標準化格式。
...
*   **生命線狀態：食物、飲水、住所 (Food, Water, Shelter)**
    ...
    *   **標準整合**：此類資料非常適合採用**人道主義交換語言 (HXL)**。透過如 `#loc+shelter`、`#capacity+persons`、`#inneed+food` 等標籤，能將來自第一線的需求評估資料立即轉換為可匯總、可分析的格式。
...
*   **生命線狀態：能源、通訊、交通 (Energy, Communications, Transportation)**
    ...
    *   **OSM整合 (關鍵)**：這是與HOT方法論整合最關鍵之處。一條中斷的道路在OSM上應標記 `practicability=no`。一棟損毀的建築應標記 `building=collapsed` 或更細緻的 `building:damage=severe`。本體論中的紀錄將直接連結至OSM的way/node ID，在地圖上提供即時的地面實況。
...
*   **職責**：此小組將作為該標準的保管人，負責版本控制、評估變更請求，並發布更新。這種正式化的流程，參考了如OGC或OASIS等國際標準制定組織的協作治理模式。一個成功的標準，其成功與否，與圍繞它建立的社群健康程度息息相關，而非僅僅取決於其技術上的完美。一個未被使用的標準，無論設計多麼精良，都是沒有價值的。採納需要來自不同利害關係人的支持：下達指令的政府官員、組織與社群領袖、基於此標準開發工具的開發者，以及使用這些工具的第一線應變人員。因此，治理模式必須設計為具備包容性與回應性，將此標準視為由整個應變社群共同擁有和共同演進的「公共財」。
